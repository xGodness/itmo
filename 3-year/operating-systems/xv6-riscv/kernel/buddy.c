...
// Reverse xor_alloc bit at position index in array
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_invert(bd_sizes[k].xor_alloc, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_invert(bd_sizes[k - 1].xor_alloc, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_invert(bd_sizes[k].xor_alloc, bi);           // free p at size k
    if (xor_bit_isset(bd_sizes[k].xor_alloc, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_invert(bd_sizes[k].xor_alloc, bi);
    }
  }
}
...
int bd_initfree_pair_right(int k, int bi) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (xor_bit_isset(bd_sizes[k].xor_alloc, bi)) {
